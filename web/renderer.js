  ...
  items.forEach((item, index) => {
    ...
    slide.innerHTML = `
      ...
      <div class="hero-content">
        <div class="hero-badge">${escapeHtml(item.Type === 'Series' ? t('hero.typeSeries') : t('hero.typeMovie'))}</div>
...
async function showDetail(base) {
  ...
  try {
    const [detail, seasonsData, similarData] = await Promise.all([
      ...
      /* Fuer eine Folge waeren "aehnliche Titel" die Nachbarfolgen —
    ...
    /* Eine Folge hat ein Querformat-Standbild, kein Hochkant-Poster.
       Im 2:3-Feld waere es bis zur Unkenntlichkeit beschnitten —
       deshalb dort das Poster der Serie, das echte Hochkant ist.
       imageUrl faellt von sich aus nur darauf zurueck, wenn die Folge
       gar kein eigenes Bild hat; hier ist der Rueckfall erwuenscht,
       gerade weil sie eins hat. */
    const seriesPoster = isEpisode && item.SeriesId && item.SeriesPrimaryImageTag
      ? imageUrl({ Id: item.SeriesId, ImageTags: { Primary: item.SeriesPrimaryImageTag } }, 'Primary', 600)
      : '';
    const poster = seriesPoster || imageUrl(item, 'Primary', 600);
    const isFavorite = Boolean(item.UserData?.IsFavorite);
    const resumeTicks = item.UserData?.PlaybackPositionTicks || 0;

    /* Bei einer Folge zaehlt, wo sie steht — "Staffel 2 · Folge 5"
       statt eines Erscheinungsjahrs, das oft ohnehin fehlt. Beide
       Schluessel gibt es schon; ein neuer waere unuebersetzt geblieben. */
    const episodeTag = isEpisode
      ? [item.ParentIndexNumber != null
           ? `${t('detail.season')} ${item.ParentIndexNumber}` : '',
         item.IndexNumber != null
           ? t('detail.episode', { number: item.IndexNumber }) : '']
    ...
    el.viewRoot.innerHTML = `
      <section class="detail-hero">
        ...
        <div class="detail-inner">
          ...
          <div class="detail-info">
            ${isEpisode && item.SeriesName ? `<button class="detail-parent" id="detail-series" type="button">${escapeHtml(item.SeriesName)}</button>` : ''}
            <h2>${escapeHtml(item.Name || '')}</h2>
      ...
      <div class="detail-tabs" id="detail-tabs">
        ...
        ${isEpisode ? `<button class="detail-tab" data-tab="siblings" type="button">${escapeHtml(t('detail.tabEpisodes'))}</button>` : ''}
    ...
    const panels = {
      ...
      siblings: () => renderSiblingEpisodes(item),
    ...
    $('detail-play').addEventListener('click', async () => {
      /* Eine Folge startet mit ihren Geschwistern als Warteschlange —
         sonst endet die Wiedergabe nach dieser einen Folge, statt
         zur naechsten weiterzulaufen. */
      if (isEpisode) {
        const eps = item.SeasonId
          ? await api(`/Shows/${item.SeriesId}/Episodes?seasonId=${item.SeasonId}&userId=${state.userId}`)
              .catch(() => null)
          : null;
        return playVideo(item, eps?.Items || []);
      }
    ...
    /* Von der Folge zurueck zur Serie — sonst waere die Folgenseite
       eine Sackgasse, aus der nur der Zurueck-Knopf herausfuehrt. */
    $('detail-series')?.addEventListener('click', () => {
...
/* Die uebrigen Folgen derselben Staffel — auf der Folgenseite tritt
   das an die Stelle der "aehnlichen Titel", die dort nur die
   Nachbarfolgen aufgezaehlt haetten. Die laufende Folge ist markiert,
   damit man sieht, wo man steht. */
async function renderSiblingEpisodes(episode) {
  const host = $('tab-content');
  host.innerHTML = `<div class="tab-panel"><div class="episode-list" id="episode-list">${skeletonEpisodes(4)}</div></div>`;
  ...
  try {
    const data = await api(
      `/Shows/${episode.SeriesId}/Episodes?seasonId=${episode.SeasonId}&userId=${state.userId}&Fields=Overview,RunTimeTicks,MediaSources,SeasonName`
    );
    ...
    episodes.forEach((entry) => {
      const row = buildEpisodeRow(entry, episodes);
      if (entry.Id === episode.Id) row.classList.add('current');
    ...
    if (!isCurrent('episodes', token)) return;
    console.error(error);
    list.innerHTML = `<div class="empty-state">${escapeHtml(t('detail.episodesFailed'))}</div>`;
  }
}
...
function openItem(item) {
  switch (item.Type) {
    ...
    /* Eine Folge fuehrt auf ihre eigene Infoseite — wie ein Film.
       ...
       Info-Knopf auf der Kachel bei Folgen wirkungslos, er spielte
       ab wie der Play-Knopf daneben. Abgespielt wird ueber Play
       (playItem), nicht ueber das Oeffnen. */
    case 'Episode':
    case 'Movie':
    default:
      navigate(() => showDetail(item));
